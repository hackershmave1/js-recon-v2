...
A compact HMAC-SHA256 token that carries the authenticated identity — user id
(``sub``), tenant id (``t``), and role — plus an expiry, signed with
``RECON_AUTH_SECRET``. STATELESS: verification needs no database; the signature proves
the server minted it and the payload names the principal. There is no per-token
revocation store — rotating the secret is the deliberate "revoke all" control.
...
CRITICAL — token domain separation: a ``"typ":"auth"`` discriminator is REQUIRED by
:func:`verify`, so any other Bearer token that happens to share this compact wire
shape (a typ-less or different-typ capability token) can never be accepted as a login,
even if signed with the same key. Verification is constant-time
(``hmac.compare_digest``) and fails CLOSED on any tampering, malformation,
missing/typed-wrong claim, or expiry.
"""
...
def verify(token: str, *, key: str, now: float | None = None) -> AuthClaims | None:
    ...
    ``typ`` (any non-``auth`` token), or expiry. Never raises on attacker input.
    """
    ...
    if not isinstance(payload, dict):
        return None
    # Require the auth discriminator: any other stray Bearer token (typ-less or a
    # different typ) can never be accepted as a login credential.
