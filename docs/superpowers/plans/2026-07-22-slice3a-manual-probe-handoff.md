...
- **Resolved during the final-gate review (was a documented limitation):** when one operation has multiple endpoint findings (same operation, different query sets), the adversarial review showed the original single-`endpoint_hash` (deterministic-first) approach would under-triage. As-built, `ReconstructedRequest` exposes `endpoint_hashes` (the sorted tuple of *all* the operation's endpoint findings) so confirming an operation can verdict every one. See the spec's "As-built amendments".
