...
| REQ-D3 | MUST | Findings are content-addressed over stable fields only (type + value + normalized path), excluding volatile col/evidence, so a retry with slightly different evidence yields the same hash. This hash keys the exactly-once outbox write (REQ-A3). [RT-04] |
